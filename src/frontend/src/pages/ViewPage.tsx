...
function RoundContent({
  ...
  const completed = round.matches.filter(
    (m) => m.result === MatchResult.completed || !!m.byePlayerId,
  ).length;
...
function PlayersSection({
  players,
}: { players: import("../backend.d").Player[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs text-right font-mono text-muted-foreground space-x-3">
        <span className="text-player-active">
          ● Active:{" "}
          {
            players.filter(
              (p) => !p.eliminated && p.status === PlayerStatus.active,
            ).length
          }
        </span>
        <span className="text-player-one-loss">
          ● 1 Loss:{" "}
          {
            players.filter(
              (p) => p.status === PlayerStatus.oneLoss && !p.eliminated,
            ).length
          }
        </span>
        <span className="text-player-eliminated opacity-60">
          ● Out: {players.filter((p) => p.eliminated).length}
        </span>
      </div>
    </div>
  );
                  ...
                  <span className="text-xs text-muted-foreground font-mono">
                    {
                      currentRound.matches.filter(
                        (m) =>
                          m.result === MatchResult.completed || !!m.byePlayerId,
                      ).length
                    }
        ...
        {tournament.status === TournamentStatus.completed && (
          <div className="space-y-8">
            ...
            <div className="text-center py-12 rounded-xl border border-gold/30 bg-gold/5 relative overflow-hidden scanlines">
                  ...
                  <div className="space-y-1">
                    {(() => {
                      const winner = players.find(
                        (p) => p.id === tournament.winner,
                      );
                      const winnerName = winner?.name ?? tournament.winner;
                      return (
                        <p className="text-4xl font-bold text-gold drop-shadow-lg">
                          {winnerName}
                        </p>
                      );
                    })()}
              ...
              {/* Decorative corners */}
              <div className="absolute top-4 left-4 text-gold/20 text-2xl">
                ♜
              </div>
              <div className="absolute top-4 right-4 text-gold/20 text-2xl">
                ♜
              </div>
              <div className="absolute bottom-4 left-4 text-gold/20 text-2xl">
                ♟
              </div>
              <div className="absolute bottom-4 right-4 text-gold/20 text-2xl">
                ♟
              </div>
            </div>
